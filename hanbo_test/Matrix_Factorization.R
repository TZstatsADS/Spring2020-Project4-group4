...
gradesc <- function(f = 10, 
                    ...
                    data, train, test){
  set.seed(0)
  #random assign value to matrix p and q    #随机设一个初始值（别太大）
  ...
  for(l in 1:max.iter){
    sample_idx <- sample(1:nrow(train), nrow(train))  #这里打乱了train的row顺序（reshuffle）
    #loop through each training case and perform update
    for (s in sample_idx){   
      
      u <- as.character(train[s,1])  # userid列
      
      i <- as.character(train[s,2])  #movieid列
      
      r_ui <- train[s,3] #rating列
      
      e_ui <- r_ui - t(q[,i]) %*% p[,u]  # t(q[,i]) %*% p[,u] 可以理解为predict “rating”  
                                         #e_ui 就是error (true rating和predict rating)
      
      grad_q <- e_ui %*% p[,u] - lambda * q[,i]              #这部分就是套公式
    ...
    #print the values of training and testing RMSE     #下面算一下RMSE，print一些结果
    if (l %% 10 == 0){
      cat("epoch:", l, "\t")
      est_rating <- t(q) %*% p       #最终的predict rating
