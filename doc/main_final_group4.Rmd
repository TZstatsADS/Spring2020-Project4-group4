...
In this step, we loaded the data and splitted the original dataset to training set(80%) and test set(20%)
```{r}
...
cat("RMSE for train set:", round(alsP3Result$train_RMSE, 4), "  RMSE for test set:", round(alsP3Result$test_RMSE, 4))
```
...
|   | A3 + KRR | A3 +R1R3 + KRR|  
| ------ | ------ | ------ | 
| training RMSE | 0.6225 | 0.5002 | 
| test RMSE | 0.9773 | 1.2418 |
By comparing RMSEs, we know that, given (A3+P3), adding (R1+R3) tends to overfit. With (R1+R3), the training RMSE is lower but the test RMSE is higher. Thus, adding (R1+R3) may do harm to the robustness and accuracy of (A3+P3).
