...
RMSE <- function(rating, est_rating){
  sqr_err <- function(id){
    sqr_error <- (as.numeric(rating[i,3]) - est_rating[as.character(rating[i,1]), as.character(rating[i,2])])^2
    return(sqr_error)
  }
  error = lapply(1:nrow(rating), sqr_err) %>% unlist()
...
change_audev = function(id, train = train, au = au,  au_dev_ui){
  user = as.character(train[id,"userId"])
  movie = as.character(train[id, "movieId"])
  au_dev_ui[user, movie] = au[,user]*train[id, "dev"]
  return(au_dev_ui)
}
```
